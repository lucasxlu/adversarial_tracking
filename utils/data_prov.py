...
class RegionDataset(data.Dataset):
    def __init__(self, img_dir, img_list, gt, configs):

        self.img_list = np.array([os.path.join(img_dir, img) for img in img_list])
        self.gt = gt

        self.batch_frames = configs['batch_frames']
        self.batch_pos = configs['batch_pos']
        self.batch_neg = configs['batch_neg']

        self.overlap_pos = configs['overlap_pos']
        self.overlap_neg = configs['overlap_neg']

        self.crop_size = configs['img_size']
